...
choices_made = [0]
current_choice = 0
for model_combination in range(1, 60):

    while current_choice in choices_made:
        user_choice = random.choice(range(1, 6))
        game_choice = random.choice(range(1, 5))
        matrix_choice = random.choice(range(1, 6))
        genre_choice = random.choice(range(1, 5))
        bert_choice = random.choice(range(1, 5))
    ...
    useravg = pd.read_csv(f"./files/user ({user_choice}).csv")
    gameavg = pd.read_csv(f"./files/game ({game_choice}).csv")
    matrix_fact = pd.read_csv(f"./files/matrix ({matrix_choice}).csv")
    ...
    with open(f'./5models/ensemble combination {model_combination}.txt','a') as file:
        print(f'\n\n\n 5 meaningfully different models randomly selected )', file=file)
        print(f'\n with these models respectively (user, game, matrix, genre, bert): {(user_choice, game_choice, matrix_choice, genre_choice, bert_choice)})', file=file)
